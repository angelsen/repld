...
**State-file layering:** three modules, one job each, in dependency order. `paths.py` decides *where* state lives (derivation from cwd, plus `--socket`/`REPLD_SOCKET` resolution). `state.py` decides *how* it's written and trusted (`atomic_write_json`, `open_private`, `read_lock`, `pid_alive`, `acquire_lock`) and imports nothing from repld. `ipc.py` is the socket layer only — `Session`, `Server`, `connect_to_kernel`. Keep it that way: `ipc.py` was previously a grab-bag five modules imported just to write a JSON file. `spawn.py` holds the one copy of the headless-kernel spawn, shared by `bridge.py` and `repld restart`; it's a separate module so the bridge (spawned every Claude Code session) doesn't have to import `lifecycle_cmd`.
