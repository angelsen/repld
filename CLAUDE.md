...
- `repld` — long-running IPython kernel in the project cwd. Writes `./.pyrepl.lock` with `{pid, socket_path}`; listens on a unix-domain socket for IPC.
...
- **Stdlib + IPython only in core.** Optional extras (`repld[browser]`, `repld[watch]`, `repld[web]`) gate anything heavier. Don't pull new deps into the base package.
