...
repld doesn't call an LLM. It doesn't decide when to stop, doesn't parse tool calls, doesn't own an agentic loop. Claude Code already does all of that — repld is a persistent kernel and an MCP server that Claude Code (or Codex, or whatever harness you're running) calls _into_. In the current jargon, that makes repld infrastructure the harness uses, not a harness itself. Calling it one would be a category error, and a confusing one — it'd read as "repld competes with Claude Code," which it doesn't.
