...
- **Per-cwd runtime dir** — the kernel's IPC path lives in `$XDG_RUNTIME_DIR/repld/projects/<slug>/kernel.lock`, where `<slug>` is `{basename}-{sha256(realpath)[:8]}`. Both the bridge and `repld exec` inherit `cwd`, resolve the same slug, read the lockfile, connect. `--socket` / `REPLD_SOCKET` override it, and every sibling file follows the socket by suffix. `repld --project DIR` / `--project-git` (env `REPLD_PROJECT` / `REPLD_PROJECT_GIT`) instead change *which* cwd that is, before anything else runs — `--project-git` is the repo's main checkout, so git worktrees can share one kernel.
